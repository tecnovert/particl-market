...
export class ImageProcessing {
    ...
    public static async convertToJPEG(imageRaw: string): Promise<string> {
        ...
        if (image.getMIME() !== 'image/jpeg') {
            const retval: any = image.getBufferAsync(Jimp.MIME_JPEG).then(buffer => buffer.toString('base64'));
            return retval;
        }
        return imageRaw;
    }
    ...
    public static async resizeImageToVersion(imageRaw: string, version: ImageVersion): Promise<string> {
        ...
        const imageBuffer: Jimp = await Jimp.read(dataBuffer);
        ...
        if (version.imageWidth > 0 && version.imageHeight > 0) {
            ...
            const resizedImage = await imageBuffer.getBufferAsync(Jimp.MIME_JPEG).then(buffer => buffer.toString('base64'));
            return resizedImage;
        }
        return imageRaw;
    }
    ...
    public static async resizeImageToFit(imageRaw: string, maxWidth: number, maxHeight: number): Promise<string> {
        ...
        if (maxWidth > 0 && maxHeight > 0 && ((imageBuffer.bitmap.width > maxWidth) || (imageBuffer.bitmap.height > maxHeight))) {
            ...
            const resizedImage = await imageBuffer.getBufferAsync(mimeType).then(buffer => buffer.toString('base64'));
            return resizedImage;
        }
        return imageRaw;
    }
    ...
    public static async resizeImageToFraction(imageRaw: string, fraction: number): Promise<string> {
        ...
        if (fraction !== 1.0) {
            ...
            const resizedImage = await imageBuffer.getBufferAsync(
                imageBuffer.getMIME() !== 'image/jpeg' ? Jimp.MIME_JPEG : 'image/jpeg'
    ...
    public static async downgradeQuality(imageRaw: string, quality: number): Promise<string> {
        const dataBuffer = Buffer.from(imageRaw, 'base64');
        let imageBuffer: Jimp = await Jimp.read(dataBuffer);
    ...
    public static async resizeImageToSize(
        imageRaw: string, byteLimit: number, scaleFraction: number = 0.9, qualityFraction: number = 0.95,
        maxIterations: number = this.MAX_COMPRESSION_ITERATIONS
    ): Promise<string> {
        let quality = 100;
        let iteration = 0;
        ...
        if (imageRaw.length > byteLimit) {
            ...
            do {
                ...
                resizedImage = await imageBuffer.getBufferAsync(imageBuffer.getMIME() !== 'image/jpeg' ? Jimp.MIME_JPEG : 'image/jpeg')
                    .then(buffer => buffer.toString('base64'));
                ...
                if (!skipQuality) {
                    ...
                    const resizedImage2 = await imageBuffer.getBufferAsync(imageBuffer.getMIME() !== 'image/jpeg' ? Jimp.MIME_JPEG : 'image/jpeg')
                        .then(buffer => buffer.toString('base64'));
