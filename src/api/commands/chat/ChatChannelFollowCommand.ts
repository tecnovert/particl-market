...
type ChannelFollowResponseErrors = CHAT_ERRORS | 'GENERIC_ERROR';


interface ChatResponse {
    success: boolean;
    errorReason?: ChannelFollowResponseErrors;
}


export class ChatChannelFollowCommand extends BaseCommand implements RpcCommandInterface<ChatResponse> {
    ...
    constructor(
        ...
        super(Commands.CHAT_CHANNEL_FOLLOW);
    ...
    public async execute( @request(RpcRequest) data: RpcRequest): Promise<ChatResponse> {
        ...
        const success = await this.chatService.followChannel(data.params[0].id, data.params[1], data.params[2])
            .catch(err => {
                this.log.error(`Following of channel ${data.params[1]} errored: `, err);
    ...
    public description(): string {
        return 'Follow a specific channel (keeps underlying objects on which the channel is based around even after they would normally have been removed).';
    }
